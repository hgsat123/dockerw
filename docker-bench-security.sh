...
main () {
  ...
  if [ -z "$exclude" ]; then
    ...
    containers=$(docker ps | sed '1d' | awk -v x=2 '{print $x}')

  else
    pattern=$(echo "$exclude" | sed 's/,/|/g')
    #containers=$(docker ps | sed '1d' | awk '{print $NF}' | grep -Ev "$pattern" )
    containers=$(docker ps | sed '1d' | awk -v x=2 '{print $x}' | grep -Ev "$pattern" )
  fi
  ...
  if [ -z "$exclude" ]; then
    ...
    containers=$(docker ps | sed '1d' |  awk -v x=2 '{print $x}' | grep -v "$benchcont")
  else
    pattern=$(echo "$exclude" | sed 's/,/|/g')
    #containers=$(docker ps | sed '1d' | awk '{print $NF}' | grep -Ev "$pattern" | grep -v "$benchcont")
    containers=$(docker ps | sed '1d' | awk -v x=2 '{print $x}' | grep -Ev "$pattern" | grep -v "$benchcont")
  fi
